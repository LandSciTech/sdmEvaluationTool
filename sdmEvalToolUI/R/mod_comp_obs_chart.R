            ...
            if (input$fill == "none") {
                ...
                if (input$groups == input$fill) {
                    ...
                    p_groups <- det |>
                        ...
                        labs(fill = tools::toTitleCase(input$groups))
                } else {
                    det <- obs() |>
                        dplyr::mutate(det = ifelse(status > 0, 1, 0)) |>
                        dplyr::group_by(
                    ...
                    p_groups <- det |>
                        ...
                        labs(fill = tools::toTitleCase(input$fill))
                }
            }
            p_groups <- p_groups +
                geom_col() +
                theme_light() +
                xlab(tools::toTitleCase(input$fill)) +
                ylab(tools::toTitleCase(input$summary))
